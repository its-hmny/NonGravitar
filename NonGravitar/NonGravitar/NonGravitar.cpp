...
void ShowConsoleCursor(bool showFlag) {
	...
	cursorInfo.bVisible = showFlag; //setta la visibilit� del cursore
...
void SolarSystemView() {
	currentSS->objectOnCanvas(player->returnPosition('x'), player->returnPosition('y'), ' '); //Vanno sostituiti con il metodo della navicella
...
bool gameOver() {
	bool userChoice = myGame->gameOver();
	system("CLS");
	myGame->~Game();
	myGame = NULL;
	currentP = NULL;
