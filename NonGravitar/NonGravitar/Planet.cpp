...
void Planet::setupSurface(int level) {
	int hPart = (rand() % 10) + 13 - level;      //altezza
	int parz = 0;                       //verso della spezzata (sale, continua ,scende)
	int i = 1;                          //equivalente alla x nel piano cartesiano
	int temp = 0;
	int molt = 0;
	bool carbSet = false;               //controlla se il carburante e' stato settato in prossimita di i
	int nextCarb = 10;                   //indica la distanza minima in cui settare il prossimo carburante
	int  valueRandCarburante = 0;

	do {
		while (i < 109) {
			do {
				parz = (rand() % 3) - 1;  //range -1 0 1
				molt = (rand() % 5) + 2;  //range  2 3 4 5 6 7
			} while (((parz * molt) + hPart > 20) || (hPart + (molt * parz) < 7 - level));
			// altezza minima        //altezza massima 
			//da tenere in considerazione che la matrice ha come (0,0) in alto a sinistra 
			if (i < nextCarb)
				carbSet = true;
			else
				carbSet = false;
	...
	} while ((countChar('O') + countChar('o') < 1 || countChar('O') + countChar('o') > 7) && elementInRow(4));     //il carburante deve essere min di 7 e max 1
}
